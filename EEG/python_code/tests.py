...
def verify(type, features, data, sub_id):
	...
	print pred, y_test
	print float(np_acc.sum())/float(len(y_test))
	if .4 <= float(np_acc.sum())/float(len(y_test)):
		print 'Verified User'
	else:
		print 'You Shall Not Pass'

def verify_intra(type, features, data, sub_id):
	intra_features = features[sub_id]
	x_train = []
	x_test = []
	y_train = []
	y_test = []
	...
	res.append(0)
	for i in range(0, len(x_test)):
		res[pred[i]] = res[pred[i]] + 1

	print res	
	if .5 <= float(np_acc.sum())/float(len(y_test)):
		print 'Verified User'
	else:
		print 'You Shall Not Pass'
