...
name = '/Users/pbm/Google Drive/THESIS/DATA/People_data/kural/calc_';
...
cal_train = [calc_f_mag_norm1;calc_f_mag_norm2;calc_f_mag_norm3];
breath_train = [breath_f_mag_norm1;breath_f_mag_norm2;breath_f_mag_norm3];
cal_full = [calc_f_mag_norm4;calc_f_mag_norm5];
breath_full = [breath_f_mag_norm4;breath_f_mag_norm5];

calc_full_mean = mean(cal_train,1);
breath_full_mean = mean(breath_train,1);

cal_k_inv = inv(cov(cal_train));
breath_k_inv = inv(cov(breath_train));
...
maha_dist_calc
maha_dist_breath
c = sum(maha_dist_calc(:,3)) / size(cal_full,1)
b = sum(maha_dist_breath(:,3)) / size(cal_full,1)
